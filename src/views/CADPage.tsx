...
import { BoltCircleCalculator } from '@/calculators/BoltCircle';
import { PolygonGenerator } from '@/calculators/PolygonGenerator';
import { useApp } from '@/context/AppContext';

const calcs = CALCULATORS.filter((c) => c.category === 'cad');

const calcMap: Record<string, React.ReactNode> = {
  'bolt-circle': <BoltCircleCalculator />,
  'polygon-generator': <PolygonGenerator />,
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 border-b border-classic-border pb-3">
        <div className="p-1.5 bg-classic-accent text-classic-accent-text"><Pencil size={18} /></div>
        <div>
          <h1 className="text-lg font-bold text-classic-text">CAD Helpers</h1>
          <p className="text-xs text-classic-muted">Bolt patterns, polygon geometry, and coordinate generation</p>
        </div>
      </div>
      {calc && (
        <nav className="flex items-center gap-2 text-xs bg-classic-panel border border-classic-border p-2">
          <button onClick={() => setActiveCalculator(null)} className="text-classic-muted hover:text-classic-text font-semibold">CAD Helpers</button>
